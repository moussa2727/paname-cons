...
const UserProcedureComponent = (): React.JSX.Element => {
  ...
  const getStepDisplayStatus = (statut: StepStatus): string => {
    return statut.toString();
  };

  const getProcedureDisplayStatus = (statut: ProcedureStatus): string => {
    return statut.toString();
  };

  const formatProcedureDateTime = (date: Date | string): string => {
    try {
  ...
  const filteredProcedures = (paginatedProcedures?.data || []).filter(
    (procedure: UserProcedure) => {
      const matchesSearch =
        ...
        getDisplayFiliere(procedure).toLowerCase().includes(searchTerm.toLowerCase());
  ...
  if (proceduresLoading && !paginatedProcedures) {
    return <LoadingScreen message='Chargement de vos procédures...' />;
  }
  ...
  return (
    <>
      ...
      >
        <main className='p-4 max-w-6xl mx-auto mt-40'>
          {proceduresLoading ? (
                          ...
                          <div className='flex items-center gap-2 mb-2'>
                            ...
                            <span
                              className={`px-2 py-1 rounded-full text-xs font-medium border shrink-0 ${
                                getProcedureStatusColor(procedure.statut).bg
                              } ${getProcedureStatusColor(procedure.statut).text} ${getProcedureStatusColor(procedure.statut).border}`}
                            >
                          ...
                          >
                            ...
                            <span
                              className={`px-2 py-1 rounded text-xs font-medium ${
                                getStepStatusColor(step.statut).bg
                              } ${getStepStatusColor(step.statut).text}`}
                            >
                      ...
                      <div className='space-y-4'>
                        <div>
                          <h3 className='text-xl font-semibold text-gray-800 mb-2'>
                            {getDisplayDestination(selectedProcedure)}
                          </h3>
                          <div className='flex items-center gap-3'>
                            <span
                              className={`px-3 py-1 rounded-full text-sm font-medium border ${
                                getProcedureStatusColor(selectedProcedure.statut)
                                  .bg
                              } ${
                                getProcedureStatusColor(selectedProcedure.statut)
                                  .text
                        ...
                        <div className='grid grid-cols-1 gap-2 text-sm'>
                          <div className='flex justify-between py-1'>
                            ...
                            <span className='text-gray-800 font-medium'>
                              {getUserFullName(selectedProcedure)}
                            </span>
                          </div>
                          <div className='flex justify-between py-1'>
                      ...
                      <div className='bg-gray-50 rounded-xl p-4'>
                        ...
                        <div className='grid grid-cols-1 gap-2 text-sm'>
                          ...
                          <div className='flex justify-between py-1'>
                            ...
                            <span className='text-gray-800 font-medium'>
                              {getDisplayFiliere(selectedProcedure)}
                            </span>
                          </div>
                        </div>
                      </div>
                      ...
                      <div className='bg-gray-50 rounded-xl p-4'>
                        ...
                        <div className='space-y-2 text-sm'>
                          <div className='flex justify-between py-1'>
                            ...
                            <span className='text-gray-800 font-medium'>
                              {formatProcedureDateTime(selectedProcedure.createdAt)}
                            </span>
                          </div>
                          {selectedProcedure.dateCompletion && (
                            <div className='flex justify-between py-1'>
                              ...
                              <span className='text-gray-800 font-medium'>
                                {formatProcedureDateTime(selectedProcedure.dateCompletion)}
                              </span>
                            </div>
                          )}
                          {selectedProcedure.updatedAt && (
                            <div className='flex justify-between py-1'>
                              ...
                              <span className='text-gray-800 font-medium'>
                                {formatProcedureDateTime(selectedProcedure.updatedAt)}
                              </span>
                            </div>
                          )}
                        </div>
                ...
                <div className='flex items-center justify-between mb-6'>
                  <div>
                    ...
                    <div className='flex items-center gap-2'>
                      <span
                        className={`px-3 py-1 rounded-full text-sm font-medium border ${
                          getProcedureStatusColor(selectedProcedure.statut).bg
                        } ${
                          getProcedureStatusColor(selectedProcedure.statut).text
                        } ${
                          getProcedureStatusColor(selectedProcedure.statut)
                            .border
                        }`}
          ...
          <div className='fixed inset-0 bg-black bg-opacity-50 flex items-end justify-center p-4 z-50 sm:items-center sm:p-6'>
            <div className='bg-white rounded-2xl p-6 w-full max-w-md max-h-[90vh] overflow-y-auto'>
              ...
              <p className='text-gray-600 mb-4'>
                ...
                {getDisplayDestination(procedureToCancel)} ?
              </p>
