// UserProfile.tsx - VERSION OPTIMISÉE (Auth & Requêtes uniquement)
...
const UserProfile = () => {
  ...
  const validateTelephone = (telephone: string): string => {
    if (telephone && telephone.trim().length < 5) {
      return 'Le téléphone doit contenir au moins 5 caractères';
    }
    return '';
  };

  // ==================== GESTION DU PROFIL OPTIMISÉE ====================
  const handleProfileSubmit = async (e: FormEvent) => {
    ...
    const telephoneError = validateTelephone(profileData.telephone);
    if (telephoneError) errors.telephone = telephoneError;

    // Vérifier les changements
    const hasChanges = profileData.email !== user.email || 
    ...
    try {
      // REQUÊTE SIMPLIFIÉE - on fait confiance au contexte
  ...
  // ==================== GESTION DU MOT DE PASSE OPTIMISÉE ====================
  const handlePasswordSubmit = async (e: FormEvent) => {
    ...
    if (!passwordData.confirmNewPassword.trim()) {
      errors.confirmNewPassword = 'La confirmation est requise';
