...
const AdminDashboard = () => {
  ...
  // UTILISATION EXCLUSIVE DE getCookie depuis le contexte
  const { getCookie, refreshAuth } = useAuth();

  // CRÉATION DE L'INSTANCE API AVEC DÉLÉGATION TOTALE AU CONTEXTE
  const dashboardApi = createDashboardApi(
    // Délégation EXCLUSIVE à getCookie du contexte
    (name: string) => {
      return getCookie(name);
    },
    // Délégation EXCLUSIVE à refreshAuth du contexte
    async () => {
            ...
            <div className='space-y-3'>
              {rendezvousStats.map((stat, index) => (
                <div key={index} className='flex items-center justify-between py-2'>
                  ...
                  <span
                    className={`px-3 py-1 rounded-full text-xs font-medium ${stat.color}`}
                  >
                    {stat.value.toLocaleString()}
                  </span>
                </div>
