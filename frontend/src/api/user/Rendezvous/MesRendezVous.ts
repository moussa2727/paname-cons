...
export class RendezvousService {
  ...
  static async confirmRendezvous(id: string): Promise<void> {
    try {
      const response = await fetch(`${API_URL}/api/rendezvous/${id}/confirm`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  static async cancelRendezvous(id: string): Promise<void> {
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          errorData.message || "Erreur lors de l'annulation du rendez-vous"
        );
      }
  ...
  // Vérifier si l'annulation est possible (2 heures comme backend)
  static canCancelRendezvous(rdv: Rendezvous): boolean {
    ...
    // STRICTEMENT supérieur à 2 heures (conforme backend)
    return hoursDiff > 2;
  }
  ...
  static getTimeRemainingMessage(rdv: Rendezvous): string | null {
    ...
    // Messages cohérents avec le backend (2 heures)
