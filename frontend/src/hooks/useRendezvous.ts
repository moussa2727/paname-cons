...
export const useUserRendezvous = (options: UseUserRendezvousOptions = {}) => {
  ...
  const fetchByEmail = useCallback(
    async (targetEmail: string) => {
      ...
      try {
        const data =
          await userRendezvousService.getRendezvousByEmail(targetEmail);
  ...
  const getRendezvousById = useCallback(
    async (id: string): Promise<RendezvousResponseDto> => {
  ...
  const getAvailableSlots = useCallback(
    async (date: Date | string): Promise<AvailableSlotsDto> => {
  ...
  const createRendezvous = useCallback(
    async (data: CreateRendezvousDto): Promise<RendezvousResponseDto> => {
  ...
  const cancelRendezvous = useCallback(
    async (id: string, reason: string): Promise<RendezvousResponseDto> => {
  ...
  useEffect(() => {
    ...
    intervalRef.current = setInterval(() => {
      if (emailRef.current && !fetchingRef.current)
        fetchByEmail(emailRef.current);
    }, refreshInterval);
    return () => {
      if (intervalRef.current) clearInterval(intervalRef.current);
    };
...
export const useAdminRendezvous = (options: UseAdminRendezvousOptions = {}) => {
  ...
  const searchRendezvous = useCallback(
    async (overrideQuery?: RendezvousQueryDto) => {
  ...
  const resetFilters = useCallback(() => {
    const defaultQuery: RendezvousQueryDto = {
      page: 1,
      limit: queryRef.current.limit ?? 20,
    };
