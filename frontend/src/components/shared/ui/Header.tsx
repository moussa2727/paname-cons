...
function Header(): React.JSX.Element {
  ...
  const handleLogout = async (): Promise<void> => {
    try {
      await logout();
      document.cookie =
        "redirect_after_login=; expires=Thu, 01 Jan 1970 00:00:00 GMT; path=/; secure; samesite=none";
    } catch (error) {
            ...
            <div className="hidden lg:flex items-center space-x-4">
              ...
              {isAuthenticated && user ? (
                ...
                <div className="flex items-center space-x-2 md:space-x-4 ml-4 md:ml-8">
                  <Link
                    to="/connexion"
                    className="flex items-center w-full sm:w-auto px-5 py-2.5 text-sm font-medium text-white bg-sky-500 hover:bg-sky-600 rounded shadow-md"
                  ...
                  <Link
                    to="/inscription"
                    className="flex items-center w-full sm:w-auto px-5 py-2.5 text-sm font-medium text-white bg-sky-600 hover:bg-sky-700 rounded shadow-md"
