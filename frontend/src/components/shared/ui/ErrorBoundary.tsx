...
class ErrorBoundary extends Component<Props, State> {
  ...
  public componentDidCatch(error: Error, errorInfo: ErrorInfo) {
    ...
    if (
      ...
      if (typeof globalThis.console !== "undefined") {
        globalThis.console.warn(
          "DOM manipulation error detected, attempting recovery...",
        );
      }
      // Try to recover by refreshing AOS if available
      if (
        typeof globalThis.window !== "undefined" &&
        (globalThis.window as { AOS?: { refresh: () => void } }).AOS
      ) {
        globalThis.setTimeout(() => {
          (
            globalThis.window as { AOS?: { refresh: () => void } }
          ).AOS?.refresh();
        }, 100);
      }
      return;
    ...
    // Handle PDF-related errors silently
    if (
      ...
      if (typeof globalThis.console !== "undefined") {
        globalThis.console.warn(
          "PDF/Document error detected, attempting recovery...",
        );
      }
      // Try to recover by reloading the page or component
