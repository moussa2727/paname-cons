...
export class SessionService {
  ...
  async create(
    ...
    this.logger.log(
      `Session created successfully for user ${this.maskUserId(userId)}`
    );
    return session;
  }
  ...
  async revoke(access_token: string, reason?: string): Promise<void> {
    this.logger.log(
      `Revoking session with token: ${this.maskToken(access_token)}`
    );
    ...
    if (result.modifiedCount > 0) {
      this.logger.log(
        `Session revoked successfully: ${this.maskToken(access_token)}`
      );
    } else {
      this.logger.warn(
        `No active session found to revoke: ${this.maskToken(access_token)}`
      );
    }
  }
  ...
  async revokeAll(userId: string, reason?: string): Promise<void> {
    this.logger.log(
      `Revoking all sessions for user ${this.maskUserId(userId)}`
    );
    ...
    this.logger.log(
      `Revoked ${result.modifiedCount} sessions for user ${this.maskUserId(userId)}`
    );
  }

  async deleteSession(access_token: string): Promise<void> {
    this.logger.log(
      `Deleting session with token: ${this.maskToken(access_token)}`
    );
    ...
    if (result.deletedCount > 0) {
      this.logger.log(
        `Session deleted successfully: ${this.maskToken(access_token)}`
      );
    } else {
      this.logger.warn(
        `No session found to delete: ${this.maskToken(access_token)}`
      );
    }
  }
  ...
  async deleteAllSessions(): Promise<{ deletedCount: number }> {
    this.logger.warn(
      `Deleting ALL sessions - this will clear the entire sessions collection`
    );

    const result = await this.sessionModel.deleteMany({}).exec();

    this.logger.log(
      `Deleted all sessions: ${result.deletedCount} sessions removed`
    );
  ...
  async deleteAllUserSessions(userId: string): Promise<void> {
    this.logger.log(
      `Deleting all sessions for user ${this.maskUserId(userId)}`
    );

    const result = await this.sessionModel.deleteMany({ user: userId });

    this.logger.log(
      `Deleted ${result.deletedCount} sessions for user ${this.maskUserId(userId)}`
    );
  }
  ...
  async getOldestSession(userId: string): Promise<Session | null> {
    this.logger.debug(
      `Getting oldest session for user ${this.maskUserId(userId)}`
    );
    ...
    this.logger.debug(
      `Oldest session query completed for user ${this.maskUserId(userId)}`
    );
    return result;
  }
  ...
  async countActiveSessions(userId: string): Promise<number> {
    this.logger.debug(
      `Counting active sessions for user ${this.maskUserId(userId)}`
    );
    ...
    this.logger.debug(
      `User ${this.maskUserId(userId)} has ${count} active sessions`
    );
    return count;
  }
  ...
  async isTokenActive(access_token: string): Promise<boolean> {
    this.logger.debug(
      `Checking if token is active: ${this.maskToken(access_token)}`
    );
    ...
    this.logger.debug(
      `Token active status: ${isActive} for ${this.maskToken(access_token)}`
    );
    return isActive;
  }
  ...
  async getActiveSessionsByUser(userId: string): Promise<Session[]> {
    this.logger.debug(
      `Getting active sessions for user ${this.maskUserId(userId)}`
    );
    ...
    this.logger.debug(
      `Found ${sessions.length} active sessions for user ${this.maskUserId(userId)}`
    );
    return sessions;
  }
  ...
  async getSessionByToken(access_token: string): Promise<Session | null> {
    this.logger.debug(
      `Getting session by token: ${this.maskToken(access_token)}`
    );
    ...
    this.logger.debug(
      `Session lookup completed for token: ${this.maskToken(access_token)}`
    );
    return session;
  }
  ...
  async updateSessionActivity(access_token: string): Promise<void> {
    this.logger.debug(
      `Updating session activity for token: ${this.maskToken(access_token)}`
    );
    ...
    if (result.modifiedCount > 0) {
      this.logger.debug(
        `Session activity updated for token: ${this.maskToken(access_token)}`
      );
    } else {
      this.logger.warn(
        `No session found to update activity for token: ${this.maskToken(access_token)}`
      );
    }
  }
  ...
  async cleanupUserSessions(userId: string, reason?: string): Promise<number> {
    ...
    const result = await this.sessionModel.updateMany(
      ...
      },
      {
        ...
        deactivatedAt: new Date(),
        revocationReason:
          reason || AuthConstants.REVOCATION_REASONS.ADMIN_CLEANUP,
      }
    );

    this.logger.log(
      `Cleaned up ${result.modifiedCount} sessions for user ${this.maskUserId(userId)}`
    );
