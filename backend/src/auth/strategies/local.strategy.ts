...
export class LocalStrategy extends PassportStrategy(Strategy) {
  ...
  async validate(email: string, password: string): Promise<any> {
    try {
      this.logger.log(
        `Tentative d'authentification locale pour l'email: ${this.maskEmail(email)}`
      );
      ...
      if (!user) {
        this.logger.warn(
          `Identifiants invalides pour: ${this.maskEmail(email)}`
        );
      ...
      if (!userId) {
        this.logger.error(
          ` Objet utilisateur sans ID pour l'email: ${this.maskEmail(email)}`,
          user
        );
        throw new UnauthorizedException(
          "Erreur interne: impossible d'extraire l'ID utilisateur"
        );
      }
      ...
      if (error instanceof UnauthorizedException) {
        ...
        if (
          errorMessage === AuthConstants.ERROR_MESSAGES.PASSWORD_RESET_REQUIRED
        ) {
          this.logger.log(
            `Réinitialisation du mot de passe requise pour l'utilisateur: ${this.maskEmail(email)}`
          );
          throw new UnauthorizedException({
        ...
        if (
          errorMessage ===
          AuthConstants.ERROR_MESSAGES.COMPTE_TEMPORAIREMENT_DECONNECTE
        ) {
          this.logger.warn(
            `Compte temporairement déconnecté pour: ${this.maskEmail(email)}`
          );
          throw new UnauthorizedException({
        ...
        this.logger.warn(
          `Erreur d'authentification pour ${this.maskEmail(email)}: ${errorMessage}`
        );
        throw error;
      }

      this.logger.error(
        `Erreur inattendue LocalStrategy pour ${this.maskEmail(email)}: ${(error as Error).message}`,
  ...
  private extractUserId(user: any): string | null {
    ...
    this.logger.error(
      `Structure d'utilisateur invalide - champs disponibles: ${Object.keys(user).join(', ')}`
    );
    return null;
  }
