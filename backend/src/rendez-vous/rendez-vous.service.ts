...
export class RendezvousService {
  ...
  async create(
    ...
    if (!user) {
      this.logger.warn(
        `Tentative de prise de rendez-vous sans compte: ${maskedEmail}`
      );
      throw new ForbiddenException(
        'Vous devez avoir un compte pour prendre un rendez-vous.'
      );
    }
    ...
    if (!isAdmin) {
      ...
      if (normalizedDtoEmail !== normalizedUserEmail) {
        this.logger.warn(
          `Tentative de création avec email différent: ${maskedEmail}`
        );
    ...
    this.logger.log(
      `Nombre de rendez-vous confirmés pour ${maskedEmail}: ${confirmedCount}`
    );

    if (confirmedCount >= 1) {
      this.logger.warn(
        `Tentative de création d'un deuxième rendez-vous pour: ${maskedEmail}`
      );
    ...
    if (processedData.destination === 'Autre') {
      if (
        !processedData.destinationAutre ||
        processedData.destinationAutre.trim() === ''
      ) {
        throw new BadRequestException(
    ...
    // Sinon, vérifier que c'est une destination valide
      ...
      throw new BadRequestException(
        `Destination invalide. Valeurs autorisées: ${validDestinations.join(', ')}, ou "Autre"`
      );
    }
    ...
    if (processedData.filiere === 'Autre') {
      if (
        !processedData.filiereAutre ||
        processedData.filiereAutre.trim() === ''
      ) {
        throw new BadRequestException(
    ...
    if (this.isToday(processedData.date)) {
      if (this.isPastTimeSlot(processedData.date, processedData.time)) {
        throw new BadRequestException(
          'Vous ne pouvez pas réserver un créneau passé'
        );
      }
    }
    ...
    this.logger.log(
      `Rendez-vous créé avec ID: ${saved._id} pour ${maskedEmail}`
    );
  ...
  async findOne(
    ...
    if (!user) {
      throw new ForbiddenException(
        "Le compte lié à ce rendez-vous n'existe plus"
      );
    }
  ...
  async update(
    ...
    if (updateDto.email && updateDto.email !== rdv.email) {
      ...
      if (!newEmailUser) {
        throw new ForbiddenException(
          'Le nouvel email doit correspondre à un compte existant'
        );
      }
    }
    ...
    if (!isAdmin && updateDto.email) {
      ...
      if (normalizedUpdateEmail !== normalizedUserEmail) {
        throw new ForbiddenException(
          "Vous ne pouvez pas changer l'email du rendez-vous"
        );
      }
    }
  ...
  async updateStatus(
    ...
    if (
      status === RENDEZVOUS_STATUS.COMPLETED &&
      avisAdmin === ADMIN_OPINION.FAVORABLE
    ) {
  ...
  async getMonthlyStats(year?: number, month?: number): Promise<any> {
    ...
    try {
      const [monthlyStats, dailyStats, comparisonStats] = await Promise.all([
  ...
  private async getPreviousMonthComparison(
    ...
    try {
      ...
      const percentage =
        previousCount > 0 ? (difference / previousCount) * 100 : 0;
  ...
  async getRealTimeStats(): Promise<any> {
    ...
    try {
      const [todayStats, hourStats, pendingConfirmations, recentChanges] =
        await Promise.all([
          // Stats d'aujourd'hui
          this.rendezvousModel.aggregate([
            {
  ...
  private async getNextHourStats(nextHour: number): Promise<any> {
    ...
    try {
      const appointments = await this.rendezvousModel
        .find({
          date: today,
          time: { $regex: `^${hourStr}:` },
          status: { $in: ['En attente', 'Confirmé'] },
  ...
  private async validateStatusTransition(
    ...
    if (newStatus === RENDEZVOUS_STATUS.CONFIRMED && !isAdmin) {
      throw new ForbiddenException(
        'Seuls les administrateurs peuvent confirmer des rendez-vous'
      );
    }
    ...
    try {
      switch (type) {
        ...
        case 'reminder':
          ...
          break;
        case 'next_day_reminder':
          await this.notificationService.sendNextDayReminder(rendezvous);
          break;
        case 'two_hour_reminder':
          await this.notificationService.sendTwoHourReminder(rendezvous);
          break;
      }
    } catch {
      this.logger.error(
        `Erreur envoi notification ${type} pour ${this.maskEmail(rendezvous.email)}`
      );
    }
  }
  ...
  @Cron('0 9 * * *') // Rappel quotidien à 9h
  async sendDailyReminders(): Promise<void> {
    ...
    for (const rdv of rendezvous) {
      ...
      this.logger.log(
        `Rappel quotidien envoyé pour: ${this.maskEmail(rdv.email)}`
      );
    }
  }

  @Cron('0 18 * * *') // Rappel la veille à 18h
  async sendNextDayReminders(): Promise<void> {
    ...
    for (const rdv of rendezvous) {
      await this.sendNotification(rdv, 'next_day_reminder');
      this.logger.log(
        `Rappel veille envoyé pour: ${this.maskEmail(rdv.email)}`
      );
    }
  }
  ...
  async sendTwoHourReminders(): Promise<void> {
    ...
    for (const rdv of rendezvous) {
      ...
      // Envoyer le rappel si le RDV est entre 2h et 2h15
      if (timeDiff > 2 * 60 * 60 * 1000 && timeDiff < 2.25 * 60 * 60 * 1000) {
        await this.sendNotification(rdv, 'two_hour_reminder');
        this.logger.log(`Rappel 2h envoyé pour: ${this.maskEmail(rdv.email)}`);
      }
    }
  }
  ...
  async autoCancelPendingRendezvous(): Promise<void> {
    ...
    if (result.modifiedCount > 0) {
      this.logger.log(
        `${result.modifiedCount} rendez-vous en attente automatiquement annulés (délai de 5h dépassé)`
      );
    }
  }
  ...
  async syncUserEmail(oldEmail: string, newEmail: string): Promise<void> {
    try {
      ...
      this.logger.log(
        `Synchronisation email: ${this.maskEmail(oldEmail)} -> ${this.maskEmail(newEmail)} - ${result.modifiedCount} rendez-vous mis à jour`
      );
    } catch (error) {
  ...
  async getCurrentUserConfirmedRendezvous(
    email: string
  ): Promise<Rendezvous | null> {
    const normalizedEmail = email.toLowerCase().trim();
