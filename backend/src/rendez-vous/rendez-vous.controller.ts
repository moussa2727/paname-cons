...
export class RendezvousController {
  ...
  @ApiOperation({
    ...
    description:
      "Créer un rendez-vous pour l'utilisateur connecté (compte requis)",
  })
  @ApiResponse({
    status: 201,
  ...
  async create(
    ...
    if (!isAdmin) {
      ...
      if (normalizedDtoEmail !== normalizedUserEmail) {
        ...
        this.logger.warn(
          `Tentative de création avec email différent: ${maskedDtoEmail} vs ${maskedUserEmail}`
        );
        throw new ForbiddenException(
          'Vous ne pouvez créer un rendez-vous que pour votre propre compte'
        );
      }
    }

    try {
      const result = await this.rendezvousService.create(
        createDto,
        userEmail,
        isAdmin
      );
      ...
      this.logger.log(
        `Rendez-vous créé avec succès: ${JSON.stringify(maskedResult)}`
      );
      return result;
    } catch (error) {
      const maskedData = this.maskSensitiveData(createDto);
      this.logger.error(
        `Erreur création rendez-vous: ${error.message}, Données: ${JSON.stringify(maskedData)}`
      );
      throw error;
    }
  }
...
async findByUser(
  ...
  @ApiOperation({
    ...
    description:
      'Récupérer les créneaux horaires disponibles pour une date spécifique',
  })
  @ApiQuery({
    name: 'date',
    required: true,
    type: String,
    description:
      'Date pour laquelle vérifier les créneaux (format: YYYY-MM-DD)',
    example: '2024-12-25',
  })
  @ApiResponse({
    status: 200,
  ...
  @ApiOperation({
    ...
    description:
      'Récupérer les dates avec des créneaux disponibles pour les 60 prochains jours',
  })
  @ApiResponse({
    status: 200,
  ...
  @ApiOperation({
    ...
    description:
      "Obtenir les détails d'un rendez-vous spécifique par son ID (compte requis)",
  })
  @ApiParam({
    name: 'id',
  ...
  async findOne(@Param('id') id: string, @Req() req: AuthenticatedRequest) {
    ...
    const isAdmin = req.user?.role === UserRole.ADMIN;
    const maskedId =
      id.length > 8 ? id.substring(0, 4) + '***' + id.slice(-4) : '***';
    const maskedUserEmail = this.maskEmail(userEmail || '');
    ...
    this.logger.log(
      `Recherche rendez-vous par ID: ${maskedId} pour utilisateur: ${maskedUserEmail}`
    );

    const rendezvous = await this.rendezvousService.findOne(
      id,
      userEmail,
      isAdmin
    );
    ...
    if (!rendezvous) {
      ...
      throw new NotFoundException(
        `Rendez-vous avec l'ID ${maskedId} non trouvé`
      );
    }
  ...
  @ApiResponse({
    status: 403,
    description: 'Non autorisé à modifier ce rendez-vous ou pas de compte',
  })
  @ApiResponse({ status: 404, description: 'Rendez-vous non trouvé' })
  async update(
    @Param('id') id: string,
    ...
    this.logger.log(
      `Modification rendez-vous ID: ${maskedId} par utilisateur: ${maskedUserEmail}, Données: ${JSON.stringify(maskedUpdateData)}`
    );
  ...
  async updateStatus(
    ...
    if (!id || id.trim() === '') {
      this.logger.warn(
        'ID de rendez-vous non fourni pour le changement de statut'
      );
      throw new BadRequestException('ID du rendez-vous requis');
    }
    ...
    if (status === RENDEZVOUS_STATUS.COMPLETED) {
      ...
      if (!avisAdmin || avisAdmin.trim() === '') {
        ...
        throw new BadRequestException(
          "L'avis admin est obligatoire pour marquer un rendez-vous comme terminé"
        );
      }
      ...
      if (!Object.values(ADMIN_OPINION).includes(avisAdmin as any)) {
        ...
        throw new BadRequestException(
          `Avis admin invalide. Valeurs autorisées: ${Object.values(ADMIN_OPINION).join(', ')}`
        );
      }
    }
    ...
    this.logger.log(
      `Changement statut rendez-vous ID: ${maskedId} par admin: ${maskedUserEmail}, Statut: ${status}, Avis: ${maskedAvis}`
    );

    return this.rendezvousService.updateStatus(id, status, avisAdmin);
  }
  ...
  @ApiOperation({
    ...
    description:
      'Annuler un rendez-vous (soft delete - changement de statut en "Annulé") (compte requis)',
  })
  @ApiParam({
    name: 'id',
    description: 'ID du rendez-vous',
  ...
  @ApiResponse({
    status: 400,
    description: "Impossible d'annuler à moins de 2h",
  })
  @ApiResponse({ status: 401, description: 'Non authentifié' })
  @ApiResponse({
    status: 403,
    description: 'Non autorisé à annuler ce rendez-vous ou pas de compte',
  })
  @ApiResponse({ status: 404, description: 'Rendez-vous non trouvé' })
  async remove(@Param('id') id: string, @Req() req: AuthenticatedRequest) {
    ...
    if (!isAdmin && !userEmail) {
      this.logger.error(
        "Email utilisateur non trouvé dans le token pour l'annulation"
      );
      throw new BadRequestException(
        'Email utilisateur non trouvé dans le token'
      );
  ...
  @ApiResponse({
    status: 400,
    description: 'Rendez-vous non en attente ou déjà passé',
  })
  @ApiResponse({ status: 401, description: 'Non authentifié' })
  @ApiResponse({
    status: 403,
    description: 'Non autorisé à confirmer ce rendez-vous',
  })
  @ApiResponse({ status: 404, description: 'Rendez-vous non trouvé' })
  async confirmRendezvous(
    @Param('id') id: string,
    @Req() req: AuthenticatedRequest
  ) {
    if (!id || id.trim() === '') {
    ...
    this.logger.log(
      `Confirmation rendez-vous ID: ${maskedId} par admin: ${maskedUserEmail}`
    );
  ...
  async checkAvailability(
    ...
    if (!id) {
      this.logger.warn(
        'ID de rendez-vous non fourni pour la vérification de disponibilité'
      );
      throw new BadRequestException('ID du rendez-vous requis');
    }
    ...
    if (!date && !time) {
      this.logger.warn(
        'Date et heure non fournies pour la vérification de disponibilité'
      );
      throw new BadRequestException(
        'Date ou heure requise pour la vérification'
      );
    }

    const rdv = await this.rendezvousService.findOne(
      id,
      req.user?.email,
    ...
    if (!rdv) {
      const maskedId =
        id.length > 8 ? id.substring(0, 4) + '***' + id.slice(-4) : '***';
      this.logger.warn(
        `Rendez-vous non trouvé pour vérification de disponibilité: ${maskedId}`
      );
    ...
    this.logger.log(
      `Vérification disponibilité pour rendez-vous ID: ${maskedId}, Date: ${checkDate}, Heure: ${checkTime}`
    );

    const isAvailable = await this.rendezvousService.checkRealTimeAvailability(
      checkDate,
      checkTime,
      id
    );
