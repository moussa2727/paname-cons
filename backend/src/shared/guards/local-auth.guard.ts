...
export class LocalAuthGuard extends AuthGuard('local') {
  ...
  handleRequest(err: any, user: any, info: any, context: ExecutionContext) {
    ...
    if (err) {
      this.logger.error(
        `Erreur dans LocalAuthGuard: ${err.message}`,
        err.stack
      );
      ...
      if (err.response && err.response.code) {
        ...
        this.logger.warn(
          `Erreur structurée détectée: ${errorCode} - ${this.maskEmail(email)}`
        );

        //  PASSWORD_RESET_REQUIRED - C'EST LE VOTRE PROBLÈME
        if (
          errorCode === AuthConstants.ERROR_MESSAGES.PASSWORD_RESET_REQUIRED
        ) {
          this.logger.warn(
            `Password reset required for: ${this.maskEmail(email)}`
          );
          throw new UnauthorizedException({
            message:
              "Un mot de passe doit être défini pour ce compte. Utilisez 'Mot de passe oublié'.",
        ...
        if (
          ...
          throw new UnauthorizedException({
            message:
              'Déconnexion administrative en cours. Réessayez plus tard.',
      ...
      if (err instanceof UnauthorizedException) {
        ...
        if (
          err.message.includes('mot de passe doit être défini') ||
          err.message.includes('PASSWORD RESET REQUIRED')
        ) {
          this.logger.warn(
            `Password reset detected from message: ${this.maskEmail(email)}`
          );
          throw new UnauthorizedException({
            message:
              "Un mot de passe doit être défini pour ce compte. Utilisez 'Mot de passe oublié'.",
      ...
      this.logger.error(
        `Erreur authentification: ${err?.message || info?.message}`
      );
      throw err;
    }
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    try {
      ...
      this.logger.log(
        ` Authentification locale réussie pour: ${this.maskEmail(email)}`
      );
      return result;
    } catch (error) {
      this.logger.error(
        ` Erreur activation guard local: ${error.message}`,
        error.stack
      );
      throw error;
    }
