...
export class UploadService {
  ...
  getFileUrl(filename: string): string {
    ...
    try {
      this.logger.log(
        `[${requestId}] Génération d'URL pour le fichier: ${this.maskFilename(filename)}`
      );
      ...
      if (!baseUrl) {
        this.logger.error(
          `[${requestId}] BASE_URL non définie dans les variables d'environnement`
        );
      ...
      this.logger.log(
        `[${requestId}] URL générée avec succès (domaine: ${this.maskDomain(cleanedBaseUrl)})`
      );

      return fileUrl;
    } catch (error) {
      this.logger.error(
        `[${requestId}] Erreur lors de la génération de l'URL: ${error.message}`
      );
      throw error;
    }
  }
